...
import io
import typing
from .types import Formats, ErrorCorrection, Colors
import traceback
import cv2
import numpy
...
async def data(data: typing.Union[str, bytes], format: Formats = Formats.PNG, version: int = 1, error_correction: ErrorCorrection = ErrorCorrection.L, box_size: int = 10, border: int = 4, fit: bool = True, optimize: int = 20, front_color: typing.Union[Colors, str] = "black", back_color: typing.Union[Colors, str] = "white"):
    ...
    try:
        ...
        qr.make_image(fill_color=front_color, back_color=back_color).save(bytes_image, format=format.value)
        return fastapi.responses.StreamingResponse(io.BytesIO(bytes_image.getvalue()), media_type=f"image/{format.value}", status_code=200)
...
async def read(qr_pic: fastapi.UploadFile):
    ...
    """
    try:
        img = cv2.imdecode(numpy.fromstring(await qr_pic.read(), numpy.uint8), cv2.IMREAD_UNCHANGED)
